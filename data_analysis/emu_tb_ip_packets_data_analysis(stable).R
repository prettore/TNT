...
trace_file_name = list.files(
  ...
  pattern = ".*Trace_Stable.csv$", # regex pattern, some explanation below -".*(1|2).*csv$",
  recursive = TRUE,          # search subdirectories
  full.names = FALSE          # return the full path
)
files_to_read = list.files(
  path = trace_folder,        # directory to search within
  pattern = ".*Trace_Stable.csv$", # regex pattern, some explanation below
  recursive = TRUE,          # search subdirectories
...
# reading test-bed summary
filter_ini <-"tb_ip_statistics_"
filter_fin <-  "_100p_20t.csv$"
statistics_folder <- paste(folder_proj,'testbed/data/statistics', sep = "")
summary_file_name = list.files(
  path = statistics_folder,        # directory to search within
  pattern = paste(filter_ini,filter_fin,sep = "(Stable).*"), # regex pattern, some explanation below
  recursive = TRUE,          # search subdirectories
  full.names = FALSE          # return the full path
)
files_to_read = list.files(
...
summary_lst_tb = lapply(files_to_read, read.csv)  # read all the matching files
for(i in 1:length(summary_lst_tb)){
  print(gsub(filter_ini,"",gsub(filter_fin,"", summary_file_name[i])))
  summary_lst_tb[[i]]$Experiment <- gsub(filter_ini,"",gsub(filter_fin,"", summary_file_name[i]))
  summary_lst_tb[[i]]$Environment <- gsub(filter_ini,"",gsub(filter_fin,"", "Test-bed"))
}


# reading test-bed metrics
filter_ini <-"tb_ip_statistics_"
filter_fin <- "_100p_20t_metrics.csv$"
...
metrics_lst_tb = lapply(files_to_read, read.csv)  # read all the matching files
for(i in 1:length(metrics_lst_tb)){
  print(gsub(filter_ini,"",gsub(filter_fin,"", metrics_file_name[i])))
  metrics_lst_tb[[i]]$Experiment <- gsub(filter_ini,"",gsub(filter_fin,"", metrics_file_name[i]))
  metrics_lst_tb[[i]]$Environment <- gsub(filter_ini,"",gsub(filter_fin,"", "Test-bed"))
}

# reading emulation summary
...
summary_lst_emu = lapply(files_to_read, read.csv)  # read all the matching files
for(i in 1:length(summary_lst_emu)){
  print(gsub(filter_ini,"",gsub(filter_fin,"", summary_file_name[i])))
  summary_lst_emu[[i]]$Experiment <- gsub(filter_ini,"",gsub(filter_fin,"", summary_file_name[i]))
  summary_lst_emu[[i]]$Environment <- gsub(filter_ini,"",gsub(filter_fin,"", "Emulation"))
}

# reading emulation metrics
filter_ini <-"ip_statistics_"
...
metrics_lst_emu = lapply(files_to_read, read.csv)  # read all the matching files
for(i in 1:length(metrics_lst_emu)){
  print(gsub(filter_ini,"",gsub(filter_fin,"", metrics_file_name[i])))
  metrics_lst_emu[[i]]$Experiment <- gsub(filter_ini,"",gsub(filter_fin,"", metrics_file_name[i]))
  metrics_lst_emu[[i]]$Environment <- gsub(filter_ini,"",gsub(filter_fin,"", "Emulation"))
}


#Summay logs
summay_results <- rbind(summary_lst_emu[[1]],summary_lst_tb[[1]])


#Packets logs
metrics <- rbind(metrics_lst_emu[[1]],metrics_lst_tb[[1]])
colnames(metrics) <- c("Time","Bitrate","Delay","Jintter","Packet loss","Round","Experiment","Environment")
metrics$Time <- as.POSIXct(metrics$Time)
metrics$Bitrate <- metrics$Bitrate/1000 

# #Buffer logs
# buffer_results <- rbind(buffer_lst[[1]],buffer_lst[[2]],buffer_lst[[3]])
# colnames(buffer_results) <- c("Time","Radio BR","Radio Buffer","Round","Experiment")
# buffer_results$Time <- as.POSIXct(buffer_results$Time)


state_sequence_ex <- rbind(trace_lst[[1]])#,trace_lst[[2]],trace_lst[[3]])
...
movingAverage <- function(data, window, column_metrics) {
  ...
  #window = 1
  #data <- metrics
  df_splitted <- split(data,list(data$Environment,data$Experiment,data$Round))
  ...
  for(experiment in df_splitted){
    ...
    test$Environment <- experiment$Environment
...
colnames(metrics) <- c("Time","Rate\n(kbit/s)","Delay\n(sec)","Jintter\n(sec)","Packet\nLoss (%)","Round","Experiment","Environment")


metrics_new_time <- replacing_time(metrics)
#buffer_new_time <- replacing_time(buffer_results)
#metrics_new_time <- metrics


#metrics_sma <- movingAverage(metrics_new_time,10,c('Rate\n(kbit/s)','Delay\n(sec)','Jintter\n(sec)','Packet\nLoss (%)'))
...
df_metrics <- rbind(melt(metrics_sma, id.vars = c("Time","Experiment","Environment")))#,
                    #melt(buffer_sma, id.vars = c("Time","Experiment")))
colnames(df_metrics) <- c("Time","Experiment","Environment","Feature","Value") 
#df_metrics <- rbind(df_metrics,state_sequence_ex)
#df_metrics$Feature = factor(df_metrics$Feature, levels=c("State","Radio BR","Radio Buffer","Bitrate","Delay","Jintter","Packet loss"))
df_metrics$Feature = factor(df_metrics$Feature, levels=c("Rate\n(kbit/s)","Packet\nLoss (%)","Delay\n(sec)","Jintter\n(sec)"))

# x axis treated as continuous variable
p <- ggplot(data=df_metrics, aes(x=Time, y=Value, color=Feature,fill=Environment)) +
  geom_line(aes(linetype=Feature),size=.4,alpha = .3) + geom_point(aes(shape = Experiment),size=.8,alpha =0.8)+
  geom_smooth(span = 0.2,linetype="dashed",color="red",size=0.5,fill="red",alpha=0.3, se=TRUE) +
  ...
  facet_grid(Feature~Environment, scales = "free")  
...
# x axis treated as continuous variable
p <- ggplot(data=df_metrics, aes(x=Time, y=Value, color=Environment,fill=Environment)) +
  geom_line(size=.4,alpha = .3) + geom_point(size=.8,alpha =0.8)+
  #geom_smooth(span = 0.2,linetype="dashed",color="red",size=0.5,fill="red",alpha=0.3, se=TRUE) +
  #scale_color_brewer(palette="Pal_grau")+
  scale_color_brewer(palette = "Dark2")+
  #scale_color_manual(values=c("gray","gray34")) +
  #scale_color_grey(start = 1, end = 0.3) +
  #scale_color_grey(start = 0.6, end = .1) +
  #scale_x_continuous(breaks= seq(0, max(df_metrics$Time), by=60)) +
  theme_minimal() + 
...
df_metrics <- rbind(melt(metrics_sma, id.vars = c("Experiment","Environment")))#,
#melt(buffer_sma, id.vars = c("Time","Experiment")))
colnames(df_metrics) <- c("Experiment","Environment","Feature","Value") 
#df_metrics <- rbind(df_metrics,state_sequence_ex)
#df_metrics$Feature = factor(df_metrics$Feature, levels=c("State","Radio BR","Radio Buffer","Bitrate","Delay","Jintter","Packet loss"))
df_metrics$Feature = factor(df_metrics$Feature, levels=c("Time","Rate\n(kbit/s)","Packet\nLoss (%)","Delay\n(sec)","Jintter\n(sec)"))
#trace_experiment <- traces_melt %>% filter(Feature != "State")
p <- ggplot(data=df_metrics, aes(x=Feature, y=Value, fill=Environment)) +
  geom_boxplot()+
  ...
  theme(legend.position="bottom",axis.text.x = element_blank(),axis.text=element_text(size=10),
...
# library(corrplot)
# 
# df_metrics_tb <- metrics_sma[which(metrics_sma$Environment =="Emulation"),]
# df_metrics_emu <- metrics_sma[which(metrics_sma$Environment =="Test-bed"),]
# df_metrics_tb <- df_metrics_tb[,c(2:5)]
# df_metrics_emu <- df_metrics_emu[,c(2:5)]
# 
# 
# #trace_experiment <- traces_melt %>% filter(Feature != "State")
# corrplot(cor(df_metrics_emu),
#          method = "number",
#          type = "upper" # show only upper side
# )
# corrplot(cor(df_metrics_tb),
#          method = "number",
#          type = "upper" # show only upper side
# )
# 
# library(correlation)
# correlation::correlation(df_metrics_emu,include_factors = TRUE, method = "auto")
