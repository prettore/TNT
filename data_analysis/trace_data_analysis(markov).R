...
traces <- rbind(trace_lst[[1]],trace_lst[[2]],trace_lst[[3]])
...
#traces_melt <- rename_labels(traces_melt,'Experiment',c("Markov","Markov_Filled","Markov_Filled_Shortest"),c("Markov (raw)","Filled","Shortest and Filled"))
#traces_melt$Experiment = factor(traces_melt$Experiment, levels=c("Markov (raw)","Filled","Shortest and Filled"))
...
traces_melt <- rename_labels(traces_melt,'Experiment',c("Markov","Markov_Filled","Markov_Filled_Shortest"),c("MR", "MF", "MSF"))
traces_melt$Experiment = factor(traces_melt$Experiment, levels=c("MR", "MF", "MSF"))
...
trace_experiment <- traces_melt %>% filter(Experiment == "MR")
# x axis treated as continuous variable
...
p <- ggplot(data=trace_experiment, aes(x=Experiment, y=Value, fill=Experiment)) +
  ...
  #scale_fill_brewer(palette="RdBu",name = "Trace", labels = c("MR", "MF", "MSF")) +
