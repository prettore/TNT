...
## reading traces
#trace_folder <- paste(folder_proj,'/military_system/data', sep = "")
#trace_file_name <- list.files(
#  path = trace_folder,        # directory to search within
#  pattern = "Trace_.*().*csv$", # regex pattern, some explanation below -".*(1|2).*csv$",
#  recursive = TRUE,          # search subdirectories
#  full.names = FALSE          # return the full path
#)
#files_to_read <- list.files(
#  path = trace_folder,        # directory to search within
#  pattern = "Trace_.*().*csv$", # regex pattern, some explanation below
#  recursive = TRUE,          # search subdirectories
...
# reading summary
filter_ini <-"tb_ip_statistics_"#"ip_statistics_"
filter_fin <-  "_1000p1to50p_20t.csv$"#"_300p1to5p_20t.csv$"#
statistics_folder <- paste(folder_proj,'/testbed_experiment/data/statistics/', sep = "")
summary_file_name <- list.files(
  path = statistics_folder,        # directory to search within
  pattern = paste(filter_ini,filter_fin,sep = "(Pendulum_Filled_Shortest|ProbRandomWalk|RandomWaypoint|Manhattan|Gauss).*"), # regex pattern, some explanation below
  recursive = TRUE,          # search subdirectories
  full.names = FALSE          # return the full path
)
files_to_read <- list.files(
...
# reading metrics
filter_ini <-"tb_ip_statistics_"#"ip_statistics_"
filter_fin <- "_1000p1to50p_20t_metrics.csv$"#"_300p1to5p_20t_metrics.csv$"#
...
# reading redio buffer from the sender node
filter_ini <-"tb_sender_buffer_"
filter_fin <- "_1000p1to50p_20t.csv$"#"_300p1to5p_20t.csv$"#
buffers_file_name <- list.files(
  path = statistics_folder,        # directory to search within
  pattern = paste(filter_ini,filter_fin,sep = "(Pendulum_Filled_Shortest|ProbRandomWalk|RandomWaypoint|Manhattan|Gauss).*"), # regex pattern, some explanation below
  recursive = TRUE,          # search subdirectories
  full.names = FALSE          # return the full path
...
buffer_lst <- lapply(files_to_read, read.csv)  # read all the matching files
for(i in 1:length(buffer_lst)){
  print(gsub(filter_ini,"",gsub(filter_fin,"", buffers_file_name[i])))
  buffer_lst[[i]]$Experiment <- gsub(filter_ini,"",gsub(filter_fin,"", buffers_file_name[i]))
}
...
#metrics$Delay <- metrics$Delay/60 
#metrics$Jitter <- metrics$Jitter/60 

#Buffer logs
buffer_results <- rbind(buffer_lst[[1]],buffer_lst[[2]])#,buffer_lst[[3]],buffer_lst[[4]])
colnames(buffer_results) <- c("Time","Radio BR","Radio Buffer","Round","Experiment")
buffer_results$Time <- as.POSIXct(buffer_results$Time)
buffer_results$`Radio BR`<- buffer_results$`Radio BR`/1000
...
# filtering the buffer metrics based on the experiment duration
filter_buffer_time <- function(data_buffer,data_metrics,extend_time) {
  
  df_splitted <- split(data_buffer,list(data_buffer$Experiment,data_buffer$Round))
  ...
  for(experiment in df_splitted){
    ...
    max_exp_time = max(data_metrics[which(data_metrics$Experiment==head(experiment$Experiment,n=1) & head(data_metrics$Round,n=1)),]$Time)
    
    max_exp_time = max_exp_time + extend_time
    
    experiment <- experiment[which(experiment$Time <= max_exp_time),]
...
#state_sequence_ex <- rename_labels(state_sequence_ex,'Experiment',c("GaussMarkov_VHF","ManhattanGrid_VHF","ProbRandomWalk_VHF","RandomWaypoint_VHF"),c("GM", "MG", "PRW","RWP"))
#metrics <- rename_labels(metrics,'Experiment',c("GaussMarkov","ManhattanGrid","ProbRandomWalk","RandomWaypoint"),c("GM", "MG", "PRW","RWP"))
#buffer_results <- rename_labels(buffer_results,'Experiment',c("GaussMarkov","ManhattanGrid","ProbRandomWalk","RandomWaypoint"),c("GM", "MG", "PRW","RWP"))

##metrics <- rename_labels(metrics,'Experiment',c("ManhattanGrid","ManhattanGrid_pred"),c("MG", "MG_Pred"))
#metrics <- rename_labels(metrics,'Experiment',c("ManhattanGrid_VHF","ManhattanGrid_VHF_pred"),c("MG", "MG_Pred"))
#buffer_results <- rename_labels(buffer_results,'Experiment',c("ManhattanGrid_VHF","ManhattanGrid_VHF_pred"),c("MG", "MG_Pred"))

metrics <- rename_labels(metrics,'Experiment',c("Pendulum_Filled_Shortest","Pendulum_Filled_Shortest_pred"),c("Pendulum", "Pendulum_Pred"))
buffer_results <- rename_labels(buffer_results,'Experiment',c("Pendulum_Filled_Shortest","Pendulum_Filled_Shortest_pred"),c("Pendulum", "Pendulum_Pred"))



colnames(metrics) <- c("Time","DR\n(kbit/s)","Delay\n(sec)","Jitter\n(sec)","P. Loss\n(count)","Round","Experiment")
colnames(buffer_results) <- c("Time","Radio DR\n(kbit/s)","Buffer\n(%)","Round","Experiment")
...
buffer_new_time <- replacing_time(buffer_results)

# filtering the buffer metrics based on the experiment duration
#buffer_new_time = filter_buffer_time(buffer_new_time,metrics_new_time,300)


# reduce the amount of data of apply sma
#metrics_sma <- movingAverage(metrics_new_time,60,c("DR\n(kbit/s)","Delay\n(sec)","Jitter\n(sec)","P. Loss\n(count)"))
#buffer_sma <- movingAverage(buffer_new_time,60,c("Radio DR\n(kbit/s)","Buffer\n(%)"))
metrics_sma <- metrics_new_time
buffer_sma <- buffer_new_time

#metrics_sma <- metrics_sma %>% sample_frac(0.30)
#buffer_sma <- buffer_sma %>% sample_frac(0.30)
...
buffer_sma$Round <- NULL



df_metrics <- rbind(melt(metrics_sma, id.vars = c("Time","Experiment")),
                    melt(buffer_sma, id.vars = c("Time","Experiment")))
colnames(df_metrics) <- c("Time","Experiment","Feature","Value") 
df_metrics$Time <- df_metrics$Time/60


#df_metrics <- rbind(df_metrics,state_sequence_ex)
#df_metrics$Feature = factor(df_metrics$Feature, levels=c("State","Nominal BR","Radio Buffer","Bitrate","Delay","Jitter","Packet loss"))
df_metrics$Feature = factor(df_metrics$Feature, levels=c("Radio DR\n(kbit/s)","DR\n(kbit/s)","Buffer\n(%)","P. Loss\n(count)","Delay\n(sec)","Jitter\n(sec)"))
#df_metrics$Experiment = factor(df_metrics$Experiment, levels=c("GM", "MG", "PRW","RWP"))


# x axis treated as continuous variable
p <- ggplot(data=df_metrics, aes(x=Time, y=Value, color=Feature,fill=Experiment)) +
  #geom_line(aes(linetype=Feature),size=.1,alpha = .3) + 
  #geom_point(shape = ".",size=.001,alpha =0.6)+
  geom_point(shape = "o",size=2)+
  geom_smooth(span = .1, method = 'loess',linetype="dashed",color="red",size=0.5,fill="red",alpha=0.3, se=FALSE) +
  ...
  theme(axis.text=element_text(size=10),
        axis.title=element_text(size=10),legend.title=element_text(size=10), #axis.text.y = element_text(angle = 30),
...
#6x4.5 or 1100x4.15
#Trace_bonn_network_statistics_gray_shaping_testbed_long
