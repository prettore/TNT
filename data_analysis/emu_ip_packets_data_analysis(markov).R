...
# reading traces
trace_folder <- paste(folder_proj,'/model_b1/data/wifi/', sep = "")
...
statistics_folder <- paste(folder_proj,'/experiment_emulation/data/statistics/sdn', sep = "")
...
for(i in seq_along(summary_lst)){
...
for(i in seq_along(metrics_lst)){
...
filter_ini <-"send_buffer_"
filter_fin <- "_100p_2s.csv$"#"_100p_20t.csv$"
...
filter_buffer_time <- function(data_buffer,pckt_metrics,extend_time) {

  df_buffer_splitted <- split(data_buffer,list(data_buffer$Experiment,data_buffer$Round))
  #df_pckt_splitted <- split(pckt_metrics,list(pckt_metrics$Experiment,pckt_metrics$Round))

  df_result <- as.data.frame(matrix(nrow = 0, ncol = length(colnames(df_buffer_splitted[[1]]))))

  #i <-0
  for(experiment in df_buffer_splitted){

    max_exp_time <- max(pckt_metrics[which(pckt_metrics$Experiment==head(experiment$Experiment,n=1) & pckt_metrics$Round==head(experiment$Round,n=1)),]$Time)
...
state_sequence_ex <- rename_labels(state_sequence_ex,'Experiment',c("Markov","Markov_Filled","Markov_Filled_Shortest"),c("MR", "MF", "MSF"))
metrics <- rename_labels(metrics,'Experiment',c("Markov_Filled","Markov_Filled_Shape"),c("MF", "MF_Shape"))
buffer_results <- rename_labels(buffer_results,'Experiment',c("Markov_Filled","Markov_Filled_Shape"),c("MF", "MF_Shape"))
...
# filtering the buffer metrics based on the experiment duration
buffer_new_time <- filter_buffer_time(buffer_new_time,metrics_new_time,1)



# reduce the amount of data of apply sma
#metrics_sma <- movingAverage(metrics_new_time,2,c('Bitrate','Delay','Jitter','Packet loss'))
...
df_metrics$Experiment = factor(df_metrics$Experiment, c("MF", "MF_Shape"))#levels=c("MR", "MF", "MSF"))

# x axis treated as continuous variable
p <- ggplot(data=df_metrics, aes(x=Time, y=Value, color=Feature,fill=Experiment)) +
  geom_line(aes(linetype=Feature),size=.4,alpha = .3) +
  geom_point(aes(shape = Experiment),size=.8,alpha =0.8)+
