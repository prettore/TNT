...
trace_file_name = list.files(
  ...
  pattern = ".*(Trace_Markov).*csv$", # regex pattern, some explanation below -".*(1|2).*csv$",
  recursive = TRUE,          # search subdirectories
  full.names = FALSE          # return the full path
)
files_to_read = list.files(
  path = trace_folder,        # directory to search within
  pattern = ".*(Trace_Markov).*csv$", # regex pattern, some explanation below
  recursive = TRUE,          # search subdirectories
...
filter_fin <-  "_2000p1to50p_20t.csv$"#"_100p_20t.csv$"
statistics_folder <- paste(folder_proj,'testbed/data/statistics/2021_TNT_2kp20t', sep = "")
summary_file_name = list.files(
  path = statistics_folder,        # directory to search within
  pattern = paste(filter_ini,filter_fin,sep = "(Markov).*"), # regex pattern, some explanation below
  recursive = TRUE,          # search subdirectories
  full.names = FALSE          # return the full path
)
files_to_read = list.files(
...
# reading metrics
filter_ini <-"tb_ip_statistics_"
filter_fin <- "_2000p1to50p_20t_metrics.csv$"#"_100p_20t_metrics.csv$"
...
#Summay logs
summay_results <- rbind(summary_lst[[1]],summary_lst[[2]],summary_lst[[3]])
...
state_sequence_ex <- rename_labels(state_sequence_ex,'Experiment',c("Markov","Markov_Filled","Markov_Filled_Shortest"),c("MR", "MF", "MSF"))
metrics <- rename_labels(metrics,'Experiment',c("Markov","Markov_Filled","Markov_Filled_Shortest"),c("MR", "MF", "MSF"))
buffer_results <- rename_labels(buffer_results,'Experiment',c("Markov","Markov_Filled","Markov_Filled_Shortest"),c("MR", "MF", "MSF"))

#colnames(metrics) <- c("Time","Data Rate\n(kbit/s)","Delay\n(sec)","Jitter\n(sec)","Packet\nLoss (count)","Round","Experiment")
#colnames(buffer_results) <- c("Time","Radio Data\nRate (kbit/s)","Radio\nBuffer (%)","Round","Experiment")
...
#df_metrics$Feature = factor(df_metrics$Feature, levels=c("State","Radio BR","Radio Buffer","Bitrate","Delay","Jitter","Packet loss"))

#df_metrics$Feature = factor(df_metrics$Feature, levels=c("Radio Data\nRate (kbit/s)","Data Rate\n(kbit/s)","Radio\nBuffer (%)","Packet\nLoss (count)","Delay\n(sec)","Jitter\n(sec)"))
df_metrics$Feature = factor(df_metrics$Feature, levels=c("Radio DR\n(kbit/s)","DR\n(kbit/s)","Buffer\n(%)","P. Loss\n(count)","Delay\n(sec)","Jitter\n(sec)"))

#df_metrics$Experiment = factor(df_metrics$Experiment, levels=c("Markov (raw)","Filled","Shortest and Filled"))
df_metrics$Experiment = factor(df_metrics$Experiment, levels=c("MR", "MF", "MSF"))
...
p <- ggplot(data=df_metrics, aes(x=Time, y=Value, color=Feature,fill=Experiment)) +
  ...
  scale_color_grey(start = 0.2, end = .1) +
...
#6x4.5 or 1100x4.15
#Trace_markov_network_statistics_gray_shaping_testbed_long
